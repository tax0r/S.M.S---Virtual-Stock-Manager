...
import json
from bot import botRandomInvest
...
def calcStonks(ticker):
    try:
        ...
        open = round(float(str(ticker.history(period="max")["Open"]).split(" ")[5].split("\n")[0]), 2)
        close = round(float(str(ticker.history(period="max")["Close"]).split(" ")[5].split("\n")[0]), 2)
    tmp1 = close - open
...
def getAvailableMoney():
    with open("files/config.json", "r") as f:
        data = json.load(f)
    return round(float(data["liquid_money"]),2)
...
def executeCmdOne(cmd, arg):
    ...
    if cmd == "info":
        ...
        print(prefix + "increase in '%' for one day (given time) | ", round(float(stonksDict["growth"]), 2), "%")
        print("\n" + prefix + "- - - - - - - - - - - - - - - - - - - - - - - - - -")
        ...
        print("\n" + prefix + "- - - - - - - - - - - - - - - - - - - - - - - - - -")
        # optional eigentlich (evlt. nach "x" menge an chars abrechen).
    ...
    if cmd == "portfolio":
        for i in getPortfolio():
            if int(i.split(" ")[1]) is not 0:
                ...
                print("(" + str(i.split(" ")[0]).upper() + ") you have invested", "{:,}".format(invested_money), "$ in", i.split(" ")[1], "share(s) =>", "{:,}".format(round(currentPrice * int(i.split(" ")[1]), 2)), "$", growth, "% growth.")
